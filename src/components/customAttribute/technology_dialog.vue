...
export default {
  ...
  methods: {
    ...
    tableChangeCheckbox(elem, row) {
      let index = selectedTechnologyList.findIndex(item => item.materialId == row.materialId);
      if (elem) {
        //新增
        if (index == -1) {
        ...
        //删除
        if (index != -1) {
          selectedTechnologyList.splice(index, 1);
        }
      }
    },
    //搜索
     ...
     */
    submitForm() {
      this.$emit("setParentTechnologyData",[...selectedTechnologyList]);
      this.closeDialog();
    },
    //关闭
    closeDialog() {
