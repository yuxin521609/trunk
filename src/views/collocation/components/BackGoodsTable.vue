...
<template>
  <div class="goods-table">
    <el-table :data="showTableData" style="width: 100%">
      ...
      <el-table-column label="返货单价" v-if="isDel" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.backUnitPrice" placeholder="请输入" size="mini" @input.native="purchasePriceChange($event, scope.row)"></el-input>
        </template>
      </el-table-column>
      <el-table-column prop="quantity" label="库存量"></el-table-column>
      <el-table-column label="数量" width="150">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.planBackQuantity"  @input.native="handleInput($event, scope)" @change="handleChange(scope)" :min="0" :max="scope.row.quantity" label="选择的数量" size="mini"></el-input-number>
        </template>
      </el-table-column>
    </el-table>
