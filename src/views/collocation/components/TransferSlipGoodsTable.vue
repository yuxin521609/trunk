      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <ul class="text-info">
            ...
            <li>款色总计：{{ checkedAllSKUInfo.typeTotal }}</li>
            <li>实际总价：{{ checkedAllSKUInfo.actualPriceTotal }}</li>
          </ul>
        </el-col>
        <el-col :span="14">
          <ul class="text-info">
            <li>
              统一折扣：<el-input v-model="checkedAllSKUInfo.discount" placeholder="请输入折扣" size="mini" class="input-w" @input.native="goodsAllDiscount($event)" @keyup.13.native="goodsAllDiscountEnter"></el-input>
              <p class="discount-remarks">输入固定格式，如7.5、6.8等，回车生效。</p>
            </li>
  ...
  export default {
    ...
    methods: {
      ...
      finalSub(orderStatus){
        ...
        this.request(this.api.order.storeAllotGoodsOrder_save, this.transferSlipAddInfo).then(data => {
          if(data && data.length){
            let html = '';
