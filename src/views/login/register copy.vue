    ...
    <div class="login-container">
      ...
      >
            ...
            <div id="register">
              ...
              >
                ...
                <el-form-item prop="phoneNum" label="手机号：">
                  <el-input id="username" v-model="ruleForm2.phoneNum" auto-complete="off" placeholder="请输入常用手机号码" maxlength="11" @blur="phonecheckBlur"/>
                </el-form-item>
                <el-form-item prop="checkCode" class="code" label="验证码：">
                  <el-input  v-model="ruleForm2.checkCode" placeholder="请输入收到的手机验证码" class="verifyCode" oninput="this.value=this.value.replace(/[^0-9]/g,'');" maxlength="6" auto-complete="new-password" />
                  <!-- 浏览器记住密码解决办法 -->
                  <el-input id="checkCode" style="position: fixed;bottom: -9999px;" />
                  <!--  -->
                  <el-button :disabled="isDisabled" size="small" class="verifyBtn" @click="validPhoneNumber">
                    {{ buttonText }}
                  </el-button>
                </el-form-item>
                <el-form-item prop="password" label="设置密码：">
                  <!-- 浏览器记住密码解决办法 -->
                  <el-input  id="password" style="position: fixed;bottom: -9999px;" type="password" />
                  <!--  -->
                  <el-input v-model="ruleForm2.password" type="password" auto-complete="new-password" minlength="8" placeholder="请设置不少于8位的登录密码"/>
                </el-form-item>
                <el-form-item prop="checkPass" label="验证密码：">
                  <!-- 浏览器记住密码解决办法 -->
                  <el-input style="position: fixed;bottom: -9999px;" type="password" />
                  <!--  -->
                  <el-input v-model="ruleForm2.checkPass" type="password" auto-complete="new-password" placeholder="请再次输入所设置的密码" />
                </el-form-item>
                <el-form-item class="line">
                  <h5 class="hint">
                    用户信息
                ...
                <el-form-item prop="name" label="姓名：">
                  <el-input v-model="ruleForm2.name" type="name" auto-complete="off" placeholder="请输入您的姓名" />
                </el-form-item>
                <!-- <el-form-item>
                  <div class="note">
                    请仔细阅读 <span class="blue">《云英新零售用户隐私》</span> 和 <span class="blue">《注册协议》</span> ，注册即表示您已充分阅读、理解并接受协议的全部内容。
                  </div>
                </el-form-item> -->
                <el-form-item>
        ...
        <div class="login-right">
          <p>已有账号，请直接点击“登录”按钮，登录使用。</p>
          <el-button style="width:100%;" class="goToLogin" native-type="button" @click="goToLogin">
            登录
          </el-button>
        </div>
      </el-form>
...
// 用户注册  发送验证码
// import { get_register, get_sendPersonCode } from '@/api/user'
...
export default {
  ...
  data() {
    ...
    //  <!--验证码是否为空-->
    const VcheckCode = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请输入收到的手机验证码'))
      } else {
        callback()
      }
    }
    ...
    const validatePass = (rule, value, callback) => {
      if (value === '' || value.length < 8) {
        callback(new Error('请设置不少于8位的登录密码'))
      } else {
    ...
    return {
      ...
      rules2: {
        ...
        phoneNum: [{ required: true, validator: checkTel, trigger: 'blur' }],
        checkCode: [{ required: true, validator: VcheckCode, trigger: 'blur' }],
        name: [{
          required: true,
          validator: (rule, value, callback) => {
            ...
            } else if (!(/^[a-zA-Z\u4e00-\u9fa5]+(\s+[a-zA-Z\u4e00-\u9fa5]+)*$/g.test(value))) {
      ...
      flag: true,
      loginPhonecheckF: false,
    }
  },
  methods: {
     phonecheckBlur() {
      if (this.ruleForm2.phoneNum != '') {
     ...
     // 校验手机号是否存在
    validPhoneNumber(fn) {
      const { phoneNum } = this.ruleForm2;
      if (phoneNum != '' && this.checkMobile(phoneNum)) {
        this.request(this.api.user.get_loginPhonecheck, { phoneNum }).then((res) => {
          this.sendCode();
    ...
    sendCode() {
      const { phoneNum } = this.ruleForm2
      this.request(this.api.user.get_sendPersonCode, { phoneNum , type: 1}).then((res) => {
        // console.log(res)
        let time = 60
        this.buttonText = '已发送'
        this.isDisabled = true
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.request(this.api.user.get_register, this.ruleForm2)
            .then((data) => {
              console.log(data)
              setTimeout(() => {
