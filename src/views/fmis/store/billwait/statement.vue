  ...
  <div>
    <div class='tab_div'>
        <el-tabs v-model="activeTabName" @tab-click="tabHandleClick">
            <el-tab-pane label="未核对" name="1"></el-tab-pane>
            <el-tab-pane label="已核对" name="2"></el-tab-pane>
        </el-tabs>
      ...
      >
        ...
        <el-table-column prop="amount" label="交易金额" align="center">
          <template slot-scope="scope">
            <span :class="{amountFount:scope.row.amount<0}">{{ scope.row.amount ?scope.row.amount:'--' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="merchant" label="收付款商户号" align="center" show-overflow-tooltip>
        ...
        <el-table-column prop="tradeTime" label="交易时间" align="center" width="170">
        ...
        <el-table-column fixed="right" label="操作">
          <template slot-scope="scope">
            <el-button @click="receiptBtnClick(scope.row)" type="text" :disabled="activeTabName=='2'" size="small">对账</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  watch: {
     activeTabName(newVal,oldVal){
      ...
      switch(this.activeTabName){
        case "1":
          this.signData.valid=false;
          break;
        case "2":
          this.signData.valid=true;
          break;
      }
  ...
  mounted() {
    this.signData.associationId=this.$route.query.statementId
  ...
  methods: {
    ...
    tabHandleClick(tab, event){
      if(this.tempTabName==this.activeTabName)return
    ...
    //批量对账按钮点击事件
    batchBtnClickEvent(){
      console.log("batchBtnClickEvent....")
      var allAmount=0
      ...
      this.$confirm("已选未对账笔数："+this.multipleSelectionData.length+"笔,对账金额：￥"+allAmount+",确认对账吗？", "批量对账", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: 'warning'
      }).then(() => {
        this.request(this.api.financial.financialData_reconciliationBatch,this.multipleSelectionData).then(res => {
          console.log(res)
    ...
    //对账Btn的事件
    receiptBtnClick(row) {
      this.$confirm("对账笔数：1笔,对账金额：+￥"+row.amount+",确认对账吗？", "对账", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: 'warning'
      }).then(() => {
        this.request(this.api.financial.financialData_reconciliation,{transactionId:row.id}).then(res => {
          console.log(res)
          // this.activeTabName='2'
          this.typeActiveIndex='1'
          this.getStatementListData();
