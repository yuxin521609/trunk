    ...
    >
      <el-row>
        <el-tabs v-model="activeName" class="basic-wrap">
          <el-tab-pane label="基本信息" name="basic">
            <el-col :span="24">
              <div class="flex-cen-start">
                <el-form-item prop="name" label="设计商名称：">
                  <el-input
                    class="input-w-2"
              ...
              <div class="flex-cen-start">
                <el-form-item prop="address" label="设计商地址：">
                  <el-input
                    class="input-w-2"
                    :disabled="showDisabled"
                    placeholder="请输入内容"
          ...
          <el-tab-pane label="设计商资质" name="brand_qcation">
            <el-col :span="24">
...
export default {
  data() {
    return {
      ...
      rules: {
        //规则验证
        name: [
          { required: true, message: "请输入设计商名称", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    arrHandleRemove(file, fileList) {
      ...
      let imgArr = this.filtersData.designCompanyProfiles;
    ...
    arrHandleAvatarSuccess(res, file) {
      ...
      let obj = new Object();
      obj.enterpriseType = 3; //1品牌 2 供应商 3设计商 4制造商
      obj.type = 1; //资质 1  协议2
      obj.path = res.data.imageUrl;
      obj.enterpriseName = "";
      this.filtersData.designCompanyProfiles.push(obj);
      // console.log( this.filtersData.designCompanyProfiles);
    }, //协议
    ...
    //获取数据
    getData() {
       this.request(this.api.product.designCompany_get, { id: this._id }).then(data => {
        let _data = data;
        let _Profiles = _data.designCompanyProfiles;
        this.filtersData = _data; //designCompanyProfiles
        this.productImages = [{ url: _data.logo }]; //logo
        //type 资质 1  协议2
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          _data.designCompanyProfiles.forEach((item, index) => {
            item.enterpriseName = _name;
          });
            this.request(this.api.product.designCompany_save, this.filtersData).then(data => {
