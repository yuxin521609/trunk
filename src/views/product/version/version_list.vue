 ...
 * @LastEditTime: 2019-11-14 15:43:36
...
<template>
  <div>
    ...
    <div class="item-margin">
      <div class="item-cont">
        <el-button-group>
          <el-button size="small" type="primary" @click="openAddDetails">新建版本</el-button>
        </el-button-group>
      </div>
      <div class="item-table">
        ...
        >
          ...
          <el-table-column prop="createTime" align="center" label="添加时间" width="100">
             <template slot-scope="scope">
              <p>{{scope.row.createTime | timestampToTime}}</p>
           </template>
          </el-table-column>
          <el-table-column prop="description" align="center" label="版本备注" min-width="100">
          </el-table-column>
          <el-table-column
            fixed="right"
...
export default {
  ...
  methods: {
       //获取列表
    getTableList() {
        this.request(this.api.version.applicationVersion_list, this.filtersData).then(data => {
        let _data = data.data;
    ...
    removeItem(item){
       this.$confirm('此操作将删除该版本, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.request(this.api.version.applicationVersion_logicDelete, item).then(data => {
               this.$message({
                  type: 'success',
                  message: '删除成功!'
                });
