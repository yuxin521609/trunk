  ...
  <div>
    ...
    <div class="item-margin">
      ...
      <div class="item-table">
        ...
        >
          <el-table-column prop="code" align="center" label="分类编码" min-width="100"></el-table-column>
          ...
          <el-table-column prop="valid" align="center" label="有效状态" min-width="80">
    ...
    <!--table END  -->
    <tp-class-details :itemParam="itemStautsParam" @refreshItem="refreshList" ref="classDetailsCpt"></tp-class-details>
  </div>
</template>
<script>
/**
 * 分类管理
 */
import ClassDetailsAy from "@/components/classification/class_details.vue";
export default {
  inject: ["reload"],
  data() {
    return {
      filtersData: {
        pageIndex: 1, //当前页码
        pageSize: 10, //每页数量
        parentId: null, //分类父级ID
        valid: null, //有效状态
        searchWords: "", //搜索关键字
        type: 1 //分类类型（1：标准分类，2：定制分类，3：原料分类或者称为属性分类）
      },
      cateArr: [],
      props: {
        value: "id",
  ...
  methods: {
    ...
    openAddClass() {
      ...
      this.itemStautsParam.title = "新增分类";
      this.$refs.classDetailsCpt.classDetailsPopupInit();
    },
    //查看  //查看 及修改 只带ID过去取消传参
    // handleShowClick(_id){
    //   this.itemStautsParam.type='show';
    //   this.itemStautsParam.title="查看分类";
  ...
  components: {
    [ClassDetailsAy.name]: ClassDetailsAy
  }
};
</script>
