  ...
  <div>
    <div class="item-margin">
      <div class="booking_top">
        <!-- 工单基本信息 -->
        <div class="overstrikings">差异单信息</div>
        <div class="user">
          <ul class="info-list">
            <li>门店：{{ get_data && get_data.storeName || '--' }}</li>
            <li>差异单号：{{ get_data &&  get_data.checkDifferenceOrderNum || '--' }}</li>
            <li>关联盘点单号：{{ get_data &&  get_data.orderNum || '--' }}</li>
            <li>盘点时间：{{ get_data &&  get_data.checkTime | timestampToTime }}</li>
            <li>财务是否确定：{{ get_data &&  get_data.financeConfirm ? '是' : '否' }}</li>
          </ul>
        </div>
        ...
        <el-table
          ...
          <el-table-column align="left" label="差异类型" min-width="130">
            <template  slot-scope="scope">{{ scope.row.differenceType ? '盘亏' : '盘盈' }}</template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getOrderDetail(id) {
        ...
        this.request(
          that.api.store.checkDifferenceOrder_getMainInfoForOrderCenter,
          {
            id
          }
        ).then(res => {
      ...
      getOrderGoodsDetail() {
        ...
        this.request(
          that.api.store.checkStockVirtualProduct_getProductInfoForOrderCenter,
          this.filtersData
        ).then(res => {
