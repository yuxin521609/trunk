   ...
   <div>
      ...
      <div class="item-margin">
         <el-form
            ...
            <el-form-item label="模糊查询：">
               <el-input v-model="filtersData.fuzzyQuery" @keyup.enter.native="handleSearch" clearable style="width:310px;" placeholder="差异单号/关联盘点单号"></el-input>
            </el-form-item>
            <el-form-item label="财务是否确认：">
              <el-select v-model="filtersData.financeConfirm" clearable>
      ...
      <div class="box">
         ...
         <el-table
            ...
            <el-table-column align="left" :show-overflow-tooltip="true" label="差异单号" min-width="80">
               <template  slot-scope="scope">{{ scope.row.checkDifferenceOrderNum || '--' }}</template>
            </el-table-column>
             <el-table-column align="left" :show-overflow-tooltip="true" label="关联盘点单号" min-width="80">
               <template  slot-scope="scope">{{ scope.row.orderNum || '--' }}</template>
            </el-table-column>
            <el-table-column align="left" :show-overflow-tooltip="true" label="门店名称" min-width="80">
               <template  slot-scope="scope">{{ scope.row.storeName || '--' }}</template>
            </el-table-column>
            <el-table-column align="left" :show-overflow-tooltip="true" label="隶属机构" min-width="80">
               <template  slot-scope="scope">{{ scope.row.organization || '--' }}</template>
            </el-table-column>
            <el-table-column align="left" :show-overflow-tooltip="true" label="盘点时间" min-width="80">
               <template  slot-scope="scope">{{ scope.row.checkTime }}</template>
            </el-table-column>
             <el-table-column align="left" :show-overflow-tooltip="true" label="财务是否确认" min-width="80">
               <template  slot-scope="scope">{{ scope.row.financeConfirm }}</template>
            </el-table-column>
            <!-- 操作 -->
            <el-table-column
               fixed="right"
   ...
   methods: {
      ...
      exportExcel() {
         ...
         require.ensure([], () => {
            ...
            export_json_to_excel(tHeader, data, "差异单列表");
         });
      },
       ...
       */
      timeRedact(item) {
         this.href('/inventory/check_record_detail',{
            id:item.id
         })
      },
      // 点击页数
      ...
      getList() {
         ...
         this.request(
         that.api.store.checkDifferenceOrder_list,
         this.filtersData
         ).then(res => {
            ...
            res.data.forEach((item, index) => {
               // 创建时间
               item.checkTime = this.$options.filters.timestampToTime(item.checkTime);
               // 财务是否确认
               item.financeConfirm = item.financeConfirm ? '是' : '否';
            });
         });
      },
      /**
