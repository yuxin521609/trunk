<template>
  <div>
    <el-dialog title="选择商品" :visible.sync="dialogTableVisible">
      <div class="box">
        <el-form class="item-form" size="small" :inline="true" :model="goodsFiltersData">
          <!-- 模糊查询 -->
          ...
          <el-form-item label="品牌：">
            <el-select
              ...
              clearable
              placeholder="请选择"
              v-model="goodsFiltersData.brandId"
            >
              <el-option
          ...
          <el-form-item label="售价：">
            <el-input
              placeholder="请输入售价最小值"
              clearable
              v-model.number="goodsFiltersData.retailPriceGE"
              class="el-input-width155"
            ></el-input>
            <el-input
              placeholder="请输入售价最大值"
              clearable
              v-model.number="goodsFiltersData.retailPriceLE"
              class="el-input-width155 ml-10"
            ></el-input>
          </el-form-item>
          <el-form-item label="季节：">
            <el-select v-model="goodsFiltersData.seasonalStyleCode" clearable placeholder="请选择">
              <el-option
                v-for="(item, index) in codeItem_list_get.data"
                :key="index"
                :label="item.name"
                :value="item.code"
              ></el-option>
          ...
          <el-form-item label="店内库存：">
            <el-input
              placeholder="请输入库存最小值"
              clearable
              v-model="goodsFiltersData.quantityLowerLimit"
              class="el-input-width155"
            ></el-input>
            <el-input
              placeholder="请输入库存最大值"
              clearable
              v-model="goodsFiltersData.quantityUpperLimit"
              class="el-input-width155 ml-10"
            ></el-input>
          </el-form-item>
          <el-form-item label="关键词：">
            <el-input
              placeholder="请输入关键词，如：货号、款号、商品名称"
              clearable
              v-model="goodsFiltersData.searchWords"
              class="el-input-width355"
            ></el-input>
          </el-form-item>
          <!-- 搜索，重置 -->
          <el-form-item>
      ...
      <div class="box">
        ...
        <div class="item-table">
          <el-button
            ...
            type="primary"
            class="ml-20 mt-24"
            @click="marketingMiddleground_addPageSingleProduct"
          >本页全部添加</el-button>
          <el-table
            class="mt-20"
            :data="goodsList.data"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <img class="discountImg" :src="scope.row.image" alt />
              </template>
            </el-table-column>
            <el-table-column
              prop
            ...
            >
              <template
                slot-scope="scope"
              >{{ scope.row.stock === null ? '门店没有该商品' : scope.row.stock }}</template>
            </el-table-column>
            <el-table-column
              prop
              align="center"
            ...
            >
              <template slot-scope="scope">
                <el-button
                  type="text"
                  v-if="scope.row.currentDiscount === null"
                  size="small"
                  @click="marketingMiddleground_addStatelessSingleProduct(scope.row)"
                >添加</el-button>
                <el-button
                  type="text"
                  v-if="scope.row.currentDiscount === 0"
                  size="small"
                  @click="marketingMiddleground_addOtherInSingleProduct(scope.row)"
                >切换到当前折扣</el-button>
                <el-button
                  type="text"
                  v-if="scope.row.currentDiscount === 1"
                  size="small"
                  @click="marketingMiddleground_removeInSingleProduct(scope.row)"
                >从列表内剔除</el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <template>
        ...
        <div class="box" v-if="discountData.type === 0">
          <el-form-item label="预约折扣类型：" prop="appointmentDiscountType">
            ...
            >
              <el-option
                v-for="(item, index) in appointmentDiscountType"
                :key="index"
                :label="item"
                :value="index"
              ></el-option>
          ...
          <el-form-item
            label="减免金额："
            v-show="discountData.appointmentDiscountType !== null && discountData.appointmentDiscountType === 0"
            prop="appointmentDiscountEd"
          >
            <el-input
              v-enter-number
              v-model="discountData.appointmentDiscountEd"
              placeholder="请输入金额，如50"
              class="el-input-width155"
            ></el-input>&nbsp;元
          </el-form-item>
          <el-form-item
            label="折扣额度："
            v-show="discountData.appointmentDiscountType !== null && discountData.appointmentDiscountType === 1"
            prop="appointmentDiscountEd"
          >
            <el-input
              v-model="discountData.appointmentDiscountEd"
              v-discount
              placeholder="请输入折扣，如3.5"
              class="el-input-width155"
            ></el-input>&nbsp;折
          </el-form-item>
        ...
        <div class="box" v-if="discountData.type === 1">
          <el-form-item label="阶梯折扣类型：" prop="ladderDiscountType">
            <el-select
              @change="changeLadderDiscountType"
              v-model="discountData.ladderDiscountType"
              class="el-input-short"
              placeholder="请选择阶梯折扣类型"
            >
              <el-option
                v-for="(item, index) in ladderDiscountType"
                :key="index"
                :label="item"
                :value="index"
              ></el-option>
          ...
          <el-form-item label="设置阶梯：" prop="ladderDiscountList">
            <div class="addRadder">
              <div v-for="(item, index) in discountData.ladderDiscountList" :key="index">
                <!-- 满数量打折 -->
                <div v-show="discountData.ladderDiscountType  === 0">
                  满
                  <el-input
                    class="el-input-mini mlr10px mb-10"
                    v-enter-number
                ...
                <!-- 满金额打折 -->
                <div v-show="discountData.ladderDiscountType  === 1">
                  满
                  <el-input
                    class="el-input-mini mlr10px mb-10"
                    v-enter-number
                ...
                <!-- 满数量减免指定金额 -->
                <div v-show="discountData.ladderDiscountType  === 2">
                  满
                  <el-input
                    class="el-input-mini mlr10px mb-10"
                    v-enter-number
                ...
                <!-- 满金额减免指定金额 -->
                <div v-show="discountData.ladderDiscountType  === 3">
                  满
                  <el-input
                    class="el-input-mini mlr10px mb-10"
                    v-enter-number
        ...
        <div v-if="discountData.type === 2">
          <div class="box">
            <el-form-item label="单品折扣类型：" prop="itemDiscountType">
              ...
              >
                <el-option
                  v-for="(item, index) in itemDiscountType"
                  :key="index"
                  :label="item"
                  :value="index"
                ></el-option>
            ...
            <el-form-item
              label="折扣额度："
              v-show="discountData.itemDiscountType !== null && discountData.itemDiscountType ===1"
              prop="itemDiscountEd"
            >
              <el-input
                placeholder="请输入折扣，如3.5"
                v-discount
                v-model="discountData.itemDiscountEd"
                class="el-input-width155"
              ></el-input>&nbsp;折
            </el-form-item>
            <el-form-item
              label="定价："
              v-show="discountData.itemDiscountType !== null && discountData.itemDiscountType === 0"
              prop="itemDiscountEd"
            >
              <el-input
                placeholder="请输入定价，如：299"
                v-enter-number
                v-model="discountData.itemDiscountEd"
                class="el-input-width155"
              ></el-input>&nbsp;元
          ...
          <div class="box" v-if="isGoods">
            <el-form class="item-form" size="small" :inline="true" :model="goodsFiltersDataCheck">
              <!-- 模糊查询 -->
              <el-form-item prop="cateIdentifier" clearable label="品类：">
                <el-cascader
                  class="input-w"
              ...
              <el-form-item label="品牌：">
                <el-select
                  ...
                  clearable
                  placeholder="请选择"
                  v-model="goodsFiltersDataCheck.brandId"
                >
                  <el-option
                    v-for="item in brandOptions"
                    :key="item.id"
              ...
              <el-form-item label="售价：">
                <el-input
                  placeholder="请输入售价最小值"
                  clearable
                  v-model="goodsFiltersDataCheck.retailPriceGE"
                  class="el-input-width155"
                ></el-input>
                <el-input
                  placeholder="请输入售价最大值"
                  clearable
                  v-model="goodsFiltersDataCheck.retailPriceLE"
                  class="el-input-width155 ml-10"
                ></el-input>
              </el-form-item>
              <el-form-item label="季节：">
                <el-select
                  v-model="goodsFiltersDataCheck.seasonalStyleCode"
                  clearable
                  placeholder="请选择"
                >
                  <el-option
              ...
              <el-form-item label="店内库存：">
                <el-input
                  placeholder="请输入库存最小值"
                  clearable
                  v-model="goodsFiltersDataCheck.quantityLowerLimit"
                  class="el-input-width155"
                ></el-input>
                <el-input
                  placeholder="请输入库存最大值"
                  clearable
                  v-model="goodsFiltersDataCheck.quantityUpperLimit"
                  class="el-input-width155 ml-10"
                ></el-input>
              </el-form-item>
              <el-form-item label="关键词：">
                <el-input
                  placeholder="请输入关键词，如：货号、款号、商品名称"
                  clearable
                  v-model="goodsFiltersDataCheck.searchWords"
                  class="el-input-width355"
                ></el-input>
              </el-form-item>
              <!-- 搜索，重置 -->
...
export default {
  data() {
    return {
      ...
      rules: {
            ...
            validator: async (rule, value, callback) => {
              ...
              if (discountData.storeId == discountData._storeId) {
                ...
                var request = await this.request(
                  this.api.market
                    .marketingMiddleground_confirmDiscountByStoreId,
                  {
                    storeId: discountData.storeId,
                    type: discountData.type,
                    discount:
                      discountData.type == 2 ? discountData.itemDiscountEd : 0,
                    discountType:
                      discountData.type == 2
                        ? discountData.itemDiscountType
                        : 0,
                  }
                );
              }
        ...
        // 折扣类型校验
        type: [
          { required: true, message: "请选择折扣方案类型！", trigger: "blur" },
        ],
        // 预约折扣类型校验
        appointmentDiscountType: [
          { required: true, message: "请选择预约折扣类型！", trigger: "blur" },
        ],
        ...
        appointmentDiscountEd: [
          {
            ...
            validator: (rule, value, callback) => {
              const appointmentDiscountType = this.discountData
                .appointmentDiscountType;
              let re = /^\d(\.\d?)?$/g;
              let res = /^\d+?$/g;
              if (!value) {
                var name = appointmentDiscountType
                  ? "请输入折扣额度!"
                  : "请输入减免金额!";
        ...
        // 阶梯折扣类型校验
        ladderDiscountType: [
          { required: true, message: "请选择阶梯折扣类型！", trigger: "blur" },
        ],
        // 阶梯折扣或金额校验
        ladderDiscountList: [
          {
        ...
        // 单品折扣类型校验
        itemDiscountType: [
          { required: true, message: "请选择单品折扣类型！", trigger: "blur" },
        ],
        // 单品折扣或金额校验
        itemDiscountEd: [
          {
  ...
  methods: {
     ...
     */
    getBrandOptions() {
      this.request(this.api.product.productBrand_list, {}, true).then(
        (data) => {
          let _data = data.data;
    ...
    marketingMiddleground_queryDiscountParameter() {
      ...
      this.request(
        ...
        if (otherDiscount.discountType === 0 && otherDiscount.type === 0) {
          ...
          otherDiscount.discountType === 0 &&
          otherDiscount.type === 1
        ) {
          ladderDiscountType = 1;
        } else if (
          otherDiscount.discountType === 1 &&
          otherDiscount.type === 0
        ) {
    ...
    createLadder() {
      ...
      //   discountType:
      //     ladderDiscountType == 0 || ladderDiscountType == 2 ? 1 : 0, // 0，满金额 1，满数量
      //   quantity: null,
      ...
      const ladderDiscountType = discountData.ladderDiscountType;
    ...
    saveDiscount() {
      ...
      this.$refs.formName.validate((v) => {
        if (v) {
          ...
          // /(\d\.\D)|(\d\.\d{2})/g
          this.discountData = JSON.parse(
            JSON.stringify(discountData).replace(
              /(\d\.\D)|(\d\.\d{2})/g,
              (value) => {
    ...
    handleSearch() {
      ...
      this.goodsFiltersData.searchWords =
        this.goodsFiltersData.searchWords &&
        this.goodsFiltersData.searchWords.replace(/(^\s)|(\s$)/g, "");
    ...
    handleSearchCheck() {
      ...
      this.goodsFiltersDataCheck.searchWords =
        this.goodsFiltersDataCheck.searchWords &&
        this.goodsFiltersDataCheck.searchWords.replace(/(^\s)|(\s$)/g, "");
