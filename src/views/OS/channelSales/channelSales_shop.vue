<template>
  <div class="base">
    ...
    <div class="content">
      <div>
        <span style="margin-right:30px;">当前机构：{{organizationName}}</span>
        <span>当前仓库：{{warehouseName}}</span>
      </div>
      <!--查询条件-->
  ...
  methods: {
      ...
      this.request(this.api.product.categories_list, {
        ...
        if (list.length > 0) {
          this.categoryList.splice(
            index + 1,
            sourceCategoryList.length - 1,
            ...list
          );
        } else {
          this.categoryList.splice(index + 1, sourceCategoryList.length - 1, {
            isSelect: true,
            list: []
          });
        }
    ...
    successBtnClickEvent() {
      ...
      for (let i = 0; i < this.bottomTableData.length; i++) {
        content +=
          this.bottomTableData[i].num * this.bottomTableData[i].retailPrice;
      }
      ...
      localStorage.setItem(
        "localTableData",
        JSON.stringify(this.bottomTableData)
      );
      localStorage.removeItem("type");
      this.$router.go(-1);
    },
    ...
    continueBtnClickEvent() {
      console.log("continueBtnClickEvent....");
      this.showZoomTable = false;
      //显示继续选择按钮
      this.showContineBtn = true;
