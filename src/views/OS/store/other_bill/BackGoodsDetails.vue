  ...
  <div class="back-goods-details item-margin item-cont">
    ...
    <el-row :gutter="20">
      <el-col :span="6">
        <ul class="text-info">
          ...
          <li>返货人：{{ backGoodsInfo.backMan ? backGoodsInfo.backMan : '--' }}</li>
      ...
      <el-col :span="6">
        <ul class="text-info">
          ...
          <li>返货总金额：¥ {{ backGoodsInfo.totalPrice }} 元</li>
      ...
      <el-col :span="6">
        <ul class="text-info">
          <li>实返件数：{{ backGoodsInfo.realityProductNum === null ? '--' : backGoodsInfo.realityProductNum}}</li>
          <li>创建时间：{{ backGoodsInfo.createTime | timestampToTime }}</li>
          <li>返货时间：{{ backGoodsInfo.backTime | timestampToTime }}</li>
        </ul>
      </el-col>
    </el-row>
    ...
    <page-table :form-data="productFormData" :get-data="getReturnProductInfoList" :table-data="tableData">
      <el-table :data="tableData.data" style="width: 100%">
        ...
        <el-table-column prop="realityBackQuantity" label="门店实返数量">
          <template slot-scope="scope">
            {{ scope.row.realityBackQuantity ? scope.row.realityBackQuantity : '--' }}
          </template>
        </el-table-column>
        <el-table-column label="实际返货金额">
          <template slot-scope="scope">
            {{ scope.row.realityBackQuantity ? '￥'+ (scope.row.backUnitPrice * scope.row.realityBackQuantity).toFixed(2) : '--' }}
          </template>
        </el-table-column>
      </el-table>
    </page-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取详情
      getReturnGoods(){
        this.request(this.api.retailStore.backGoods_get, {id: this._id}).then(data => {
          this.backGoodsInfo = data;
        });
      },
      // 获取商品详情
      getReturnProductInfoList(){
        this.request(this.api.retailStore.backProductInfo_list, this.productFormData).then(data => {
          this.tableData = data;
        });
      },
      // 状态转name
