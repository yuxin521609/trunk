...
<template>
   <div>
      <!-- 转派 -->
      <el-dialog title="转派工单处理人员" :visible.sync="dialogTableVisible">
      ...
      <div class="item-margin">
         <div class="item-table">
            <el-table
               ...
               <!-- 工单名称 -->
               <el-table-column align="center" label="工单名称" min-width="150">
                   <template  slot-scope="scope">{{ scope.row.name || '--' }}<span class="warningType" :style="{background:scope.row.alarmColor}" v-if="scope.row.alarmStatus">{{ scope.row.alarmStatus  }}</span></template>
               </el-table-column>
               <!-- 优先级 -->
               ...
               <!-- 接单人 -->
               <el-table-column align="center" label="接单人" min-width="90">
               ...
               <el-table-column
                  ...
                  width="180">
                  <template slot-scope="scope">
                     <el-button type="text" @click="usersGet(scope.row)" size="small">转派</el-button>
                     <el-button type="text" @click="dispose(scope.row, true)" size="small">查看详情</el-button>
                     <el-button type="text" size="small" @click="waitWorkOrderRevokeTransfer(scope.row)">撤销指派</el-button>
                  </template>
               </el-table-column>
            </el-table>
...
export default {
   name:'workOrder_beingProcessed_list',
   data() {
      return{
         ls:{},
         usersGetList:[],
   ...
   methods: {
      ...
      fnDialogTableVisible() {
         this.waitWorkOrderTransfer(this.ls);
      ...
      waitWorkOrderTransfer(obj) {
         ...
         this.request(
            this.api.job_order.waitWorkOrderTransfer,
       ...
       * 处理中-撤销指派
       */
      waitWorkOrderRevokeTransfer(obj) {
         const that = this;
         ...
         this.$confirm('确定要撤销吗？', '提示', {
          ...
          type: 'warning'
            ...
            this.request(
               this.api.job_order.waitWorkOrderRevokeTransfer,
               obj
            ).then(res => {
               this.$message({
                  type: 'success',
