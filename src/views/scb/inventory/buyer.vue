  ...
  <div>
      ...
      <el-form class="item-form" size="small" :inline="true" :model="filtersData">
        ...
        <el-form-item label="模糊查询：">
          <el-input
            ...
            style="width:310px;"
            placeholder="商品款号/商品货号/商品名称/关联单据号/往来对象名称"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="收发类型：">
          <el-select v-model="filtersData.typeCode" clearable filterable>
            <el-option
    ...
    <div class="box">
      ...
      >
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="结算单价" min-width="140">
          <template slot-scope="scope">{{ scope.row.reservationOrderNum || '--' }}</template>
        </el-table-column>
        <el-table-column align="left" :show-overflow-tooltip="true" label="收发类型" min-width="120">
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="往来对象名称" min-width="60">
      ...
      >
        <div v-if="getList_data.data && getList_data.data.length > 0">
          累计收货数：<span>{{ storeRetailOrder_pcListStatistics_data.totalCount }}</span>
          累计收货货值：<span>{{ storeRetailOrder_pcListStatistics_data.saleSumProQty }}</span>
          累计发货数：<span>{{ storeRetailOrder_pcListStatistics_data.orderSumProAmount }}</span>
          累计发货货值：<span>{{ storeRetailOrder_pcListStatistics_data.orderSumAmount }}</span>
          累计核销数：<span>{{ storeRetailOrder_pcListStatistics_data.orderSumAmount }}</span>
          累计核销货值：<span>{{ storeRetailOrder_pcListStatistics_data.orderSumAmount }}</span>
        </div>
        <!-- 分页 -->
        <el-pagination
          class="mr-20"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.request(
        that.api.scb.receiveShippingDetails_list,
        this.filtersData
      ).then(res => {
