<template>
  <div class="box">
    <el-dialog title="选择商品" :visible.sync="dialogTableVisible">
      ...
      <div>
        ...
        <div class="item-table">
          <page-table :form-data="goodsFiltersData" :get-data="purchasingAgentProduct_scbProductToProductDiscount" :table-data="goodsList">
            <el-table
              class="mt-20"
              :data="goodsList.data"  
              stripe
              ...
              <el-table-column
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button type="text" v-if="scope.row.currentMerchantDiscount === 1" size="small" @click="purchasingAgentDiscount_deleteSPUFromDiscount(scope.row.code)">从列表内剔除</el-button>
                  </template>
              </el-table-column>
            </el-table>
          </page-table>
    ...
    <!-- 指定商品折扣 -->
    <h4>指定商品折扣</h4>
    <el-input class="mt-20" disabled value="设置后，以下商品将按照指定折扣进行结算。"></el-input>
    <div class="mt-20">指定商品折扣：{{ purchasingAgentDiscount_get_data.productDiscountValue }}折<el-button size="mini" type="success" plain class="ml-20" @click="open(purchasingAgentDiscount_get_data.productDiscountValue)">修改</el-button></div>
    <!-- 货号信息 -->
    <h4 class="mt-20">商品列表</h4>
    <div class="item-table">
      <el-button type="primary" @click="addProduct" class="mt-30">添加</el-button>
      <page-table :form-data="filtersData" :get-data="purchasingAgentProduct_productDiscountProductList" :table-data="goodsData">
        <el-table
          class="mt-20 table"
          :data="goodsData.data"
          stripe
        ...
        <el-table-column prop="" align="center" :show-overflow-tooltip="true" label="商品名称" min-width="100">
          ...
          <el-table-column
            ...
            width="180">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="purchasingAgentDiscount_deleteSPUFromDiscount(scope.row.productCode)">剔除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </page-table>
...
export default {
  data() {
    return {
      ...
      // 获取折扣详情
      purchasingAgentDiscount_get_data:{},
      // 选中某个折扣
      purchasingAgentDiscount_get_data:{},
      // 检索全部商品
      goodsFiltersData:{
  ...
  created() {
    ...
    this.getClassData(); // 获取品类
    this.purchasingAgentPriceManagement_getById(); // 获取商户详情
    this.purchasingAgentDiscount_get(); // 获取折扣详情
  },
  methods: {
     /**
     * 获取品牌
    ...
    addProduct() {
      ...
      Object.assign(this.filtersData, {
        "purchaseAgentId":this.purchasingAgentDiscount_get_data.purchaseAgentId,
        "productDiscountType":"1",
        "productDiscountValue":this.purchasingAgentDiscount_get_data.productDiscountValue + ''
      });
    ...
    purchasingAgentDiscount_transferSPU(item) {
      const that = this;
      const text = `将从”${ item.currentMerchantDiscountValue }折”转移到“${ this.purchasingAgentDiscount_get_data.productDiscountValue }折”，是否继续？`;
      ...
      this.$confirm(text, '提示', {
        ...
        this.request(
          this.api.scb.purchasingAgentDiscount_transferSPU,
          {
            ...
            "productDiscountId": this.purchasingAgentDiscount_get_data.id,
     ...
     *btn 剔除
     */
    purchasingAgentDiscount_deleteSPUFromDiscount(code) {
      const that = this;
     ...
     * 获取折扣详情
     */
    purchasingAgentDiscount_get(item) {
      const that = this;

      this.request(
        this.api.scb.purchasingAgentDiscount_get,
        {
          id:this.$route.query.discountId
        },
        true
      ).then(res => {
        that.purchasingAgentDiscount_get_data = res;
        this.goodsFiltersData.productDiscountValue = res.productDiscountValue;
        this.goodsFiltersData.purchaseAgentId = res.purchaseAgentId;
        this.purchasingAgentProduct_productDiscountProductList();// 已添加商品
      });
    },
    /**
     * 修改折扣
     ...
     */
    purchasingAgentDiscount_save(value) {
      const that = this;
      ...
      this.request(
        ...
        {
          id:this.$route.query.discountId,
          priceManagementId:this.$route.query.id,
          "productDiscountType": 1,
        ...
        that.purchasingAgentDiscount_get_data = res;
        // 修改完折扣重新刷新列表
        this.purchasingAgentDiscount_get();
      });
    },
    /**
     * 修改某条折扣
    ...
    checkDiscount(row) {
      ...
      this.purchasingAgentDiscount_get_data = row;
      this.purchasingAgentProduct_productDiscountProductList();// 已添加折扣商品
    },
    /**
     * （获取季节）
    ...
    open(inputValue) {
      this.$prompt('', '修改采购折扣', {
       ...
       this.purchasingAgentDiscount_save(value);
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '取消输入'
    ...
    handleSearch() {
      ...
      this.purchasingAgentProduct_scbProductToProductDiscount();
    },
    /**
     * 搜索重置
     */
