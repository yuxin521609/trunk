...
<template>
  <div class="goods-table">
    <el-table :data="showTableData" style="width: 100%">
      ...
      <!-- <el-table-column prop="allotPrice" label="调拨结算单价" width="100"></el-table-column> -->
      <el-table-column prop="stock" label="未核销数量"></el-table-column>
      <el-table-column label="核销数量" width="150">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.quantity"  @input.native="handleInput($event, scope)" @change="handleChange(scope)" :min="0" label="选择的数量" size="mini" :class="{'txt-red' : scope.row.stock < scope.row.quantity}"></el-input-number>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="item-cont clearfix">
      <ul class="text-info">
        <!-- <li>核销吊牌总价：{{ checkedAllSKUInfo.retailPriceTotal }}</li> -->
        <li>核销sku种类数：{{ checkedAllSKUInfo.skuNum }}</li>
        <li>核销商品总数：{{ checkedAllSKUInfo.goodsTotal }}</li>
      ...
      <div class="btns">
        ...
        <el-button type="primary" @click="finalSub('0')" size="small">确认核销</el-button>
      </div>
    </div>
  </div>
</template>
      ...
      getCheckedAllSKUInfo(){
        ...
        if(this.tableData.length){
          this.tableData.forEach(item => {
            ...
            this.checkedAllSKUInfo.transferValueCopy += Number(item.price) * Number(item.quantity);
            ...
            this.checkedAllSKUInfo.actualPriceTotal += Number(item.stock) * Number(item.quantity);
      ...
      finalSub(orderStatus){
        ...
        this.tableData.forEach(item => {
          let obj = {
            ...
            // 数量
            sendReceiveQuantity: item.quantity,
            stockQuantity: item.stock,
          };
          item.sendReceiveQuantity = item.quantity;
          item.stockQuantity = item.stock;
          // item.channelCustomerId = this.transferSlipAddInfo.outStoreId;
        ...
        this.request(this.api.scb.channelWriteOffSettle_writeOff, {
          writeOffTotal:this.checkedAllSKUInfo.goodsTotal,
          writeOffValue:this.checkedAllSKUInfo.transferValueCopy,
          channelCustomerId:this.transferSlipAddInfo.outStoreId, 
          sendReceiveRecordList:this.tableData
        }).then(data => {
          if(data && data.length){
            let html = '';
