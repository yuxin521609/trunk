  ...
  <div>
    ...
    <div class="item-margin">
      <el-form class="item-form" size="small" :inline="true" :model="filtersData">
        ...
        <el-form-item label="收发类型：">
          <el-select v-model="filtersData.sendReceiveName" clearable filterable>
            <el-option
        ...
        <el-form-item label="时间：" prop="startDate">
          <el-date-picker value-format="yyyy-MM-dd 00:00:00" v-model="filtersData.startTime" type="date" placeholder="开始日期"></el-date-picker>
        </el-form-item>
        <el-form-item prop="endDate">
          <el-date-picker value-format="yyyy-MM-dd 23:59:59" v-model="filtersData.endTime" type="date" placeholder="结束日期"></el-date-picker>
        </el-form-item>
        <!-- 搜索，重置 -->
        <el-form-item>
    ...
    <div class="box">
      ...
      >
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="时间" min-width="60">
        ...
        <el-table-column align="left" :show-overflow-tooltip="true" label="收发类型" min-width="60">
          <template slot-scope="scope">{{ scope.row.sendReceiveName }}</template>
        </el-table-column>
        <!-- 新增 -->
        <el-table-column align="left" :show-overflow-tooltip="true" label="收发数量" min-width="60">
      ...
      <div class="pagination" :class="{ 'justify-content': getList_data.dataList.data && getList_data.dataList.data.length > 0 }">
        <div v-if="getList_data.dataList.data && getList_data.dataList.data.length > 0">
          累计收货数：<span>{{ getList_data.info.receiveTotal }}</span>
          累计收货货值：<span>{{ getList_data.info.receiveValue }}</span>
          累计返货数：<span>{{ getList_data.info.returnTotal }}</span>
          累计返货货值：<span>{{ getList_data.info.returnValue }}</span>
          累计核销数：<span>{{ getList_data.info.writeOffTotal }}</span>
          累计核销货值：<span>{{ getList_data.info.writeOffValue }}</span>
        </div>
      <!-- 分页 -->
      <el-pagination
        class=""
  ...
  methods: {
    ...
    exportExcel() {
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "下游渠道收发商品记录");
      });
    },
    ...
    getList() {
      ...
      this.request(
        that.api.scb.channelSendReceiveRecord_dataList,
        this.filtersData
      ).then(res => {
        ...
        res.dataList.data.forEach((item, index) => {
          ...
          // item.sendReceiveType = this.sendReceiveList(item.sendReceiveType);
        });
      });
    },
    /**
