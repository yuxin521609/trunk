...
<template>
  <div class="transfer-slip-add-goods">
    ...
    <div class="side">
      <div class="item-cont">
        <el-button size="mini" type="primary" @click="settleQuery_findSettle">已选核销商品</el-button>
        <el-upload class="upload-goods-btn" :action="uploadGoodsUrl" :show-file-list="false" :auto-upload="false" :on-change="uploadGoodsChange">
          <el-button size="mini" type="primary" plain>导入核销商品</el-button>
        </el-upload>
        ...
        <div>
          <el-form class="item-form" :model="formData" label-width="90px" size="small" ref="searchForm" inline :rules="rules">
            ...
            <el-form-item label="品牌：" prop="brandId">
              <el-select class="input-w" placeholder="请选择" v-model="formData.brandName" clearable filterable>
                <el-option v-for="item in brandOptions" :key="item.id" :label="item.name" :value="item.name"></el-option>
              </el-select>
            </el-form-item>
           <el-form-item label="季节：" prop="brandId">
              <el-select class="input-w" placeholder="请选择" v-model="formData.seasonalStyle" clearable filterable>
                <el-option v-for="item in codeItem_list_get.data" :key="item.id" :label="item.name" :value="item.name"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="货主：" prop="brandId">
              <el-select class="input-w" placeholder="请选择" v-model="formData.ownerId" clearable filterable>
                <el-option v-for="item in warehouseBase_goodsOwner_data.data" :key="item.id" :label="item.name" :value="item.code"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
          ...
          if(data.code && data.code == 200){
            ...
            if(goodsData && goodsData.length > 0){
              for(let n = 0; n < goodsData.length; n++){
                ...
                if(goodsIndex !== -1){
                  this.checkedSKUTableData[goodsIndex].quantity = goodsData[n].inputQuantity;
                }else {
                  goodsData[n].allotPrice = goodsData[n].retailPrice;
                  goodsData[n].stock = goodsData[n].quantity;
                  goodsData[n].quantity = goodsData[n].inputQuantity;
